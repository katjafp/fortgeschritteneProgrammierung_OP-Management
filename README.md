...
## Tests ausführen
Die drei Testdateien bauen sich jeweils ihr eigenes, unabhängiges Test-Setup auf (unabhängig von klinik_setup.py) und laufen einzeln:

```bash
python testressource.py
python testop.py
...
Den Überblick über alles hat der `OPManager`: Er verbindet die Ressourcen mit der Saal-Zeitplanung, prüft bei jeder Buchung, ob alles verfügbar ist, und kümmert sich beim Verschieben einer OP darum, dass Folgetermine korrekt mitgezogen werden.

`klinik_setup.py` baut einmalig die komplette Beispiel-Klinik auf (Säle, Personal, Geräte, Siebe, Lager, OP-Typen) und wird sowohl von main.py als auch von `app.py` genutzt – so gibt es nur eine Stelle im Code, an der die Stammdaten gepflegt werden müssen.

`main.py` ist mein Konsolen-Testszenario (ein kompletter Beispieltag mit 5 OPs über 3 Säle, inkl. Ressourcenkonflikt, Live-Status, Komplikation mit Zeitverschiebung und Meldebestand-Warnung), app.py die Streamlit-Oberfläche für die Live-Demo.
...
nachfolgenden OPs im selben Saal um denselben Betrag (in beide Richtungen: kürzer = alle rücken vor, länger = alle rücken nach hinten). Vor der eigentlichen Änderung wird zweifach geprüft:
